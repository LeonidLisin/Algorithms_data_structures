...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    private Node<E> root;
    private int size, maxHeight;

    TreeImpl(int maxHeight){
    ...
    private boolean isLocalTreeBalanced(Node node) {
        return (node == null) ||
                isLocalTreeBalanced(node.getLeftChild()) &&
                        isLocalTreeBalanced(node.getRightChild()) &&
                        Math.abs(height(node.getLeftChild()) - height(node.getRightChild())) <= 1;
    }

    @Override
    public boolean isBalanced(){
        return isLocalTreeBalanced(root);
    }
