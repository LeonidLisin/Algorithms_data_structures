...
public class Graph {
    ...
    public ArrayList<Vertex> findShortestWay(String a, String b){
        int startIndex = indexOf(a), finishIndex = indexOf(b);
        int minWay = size;
        if (startIndex == -1 || finishIndex == -1) {
            throw new IllegalArgumentException("Invalid label");
        }
        ArrayList<Vertex> way = new ArrayList<>(), bestWay = new ArrayList<>();
        if (a.equals(b)){
            way.add(vertexList.get(startIndex));
            System.out.println("Departure concurs destination");
            System.out.println("___________");
            return way;
        }
        Queue<Vertex> queue = new ArrayDeque<>();
        Vertex vertex = vertexList.get(startIndex), step;
        queue.add(vertex);
        ...
        resetVertexState();
        if (bestWay.size() == 0) {
            System.out.println("No available ways from " + vertexList.get(startIndex).getLabel() +
                    " to " + vertexList.get(finishIndex).getLabel());
            System.out.println("___________");
            ...
            System.out.println("The shortest way from " + vertexList.get(startIndex).getLabel() +
                    " to " + vertexList.get(finishIndex).getLabel() + " is:");
            for (Vertex bw : bestWay)
                System.out.println(bw.getLabel());
            System.out.println("___________");
            return bestWay;
        }
    }
